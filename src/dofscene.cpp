...
void DofScene::initFBO2() {
    // First delete the FBO if it has been created previously
    glBindFramebuffer(GL_FRAMEBUFFER, m_fboId2);
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER)==GL_FRAMEBUFFER_COMPLETE) {
        glDeleteTextures(1, &m_fboTextureId2);
        glDeleteTextures(1, &m_fboDepthId2);
        glDeleteFramebuffers(1, &m_fboId2);
    }
    glBindFramebuffer(GL_FRAMEBUFFER, 0);

    // Generate a texture to write the FBO result to
    glGenTextures(1, &m_fboTextureId2);
    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D, m_fboTextureId2);
    glTexImage2D(GL_TEXTURE_2D,
                 0,
                 GL_RGB,
                 m_width,
    ...
    // The depth buffer is rendered to a texture buffer too
    glGenTextures(1, &m_fboDepthId2);
    glActiveTexture(GL_TEXTURE2);
    glBindTexture(GL_TEXTURE_2D, m_fboDepthId2);
    glTexImage2D(GL_TEXTURE_2D,
                 0,
                 GL_DEPTH_COMPONENT,
                 m_width,
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, m_fboId2);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_fboTextureId2, 0);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, m_fboDepthId2, 0);
...
void DofScene::paintGL() noexcept {
    ...
    // Needed for colour cycling
    colourStep = 1.0f / float(m_numObjects);
    ...
    for (i=0; i<m_numObjects; ++i) {
        ...
        // Note the matrix multiplication order as we are in COLUMN MAJOR storage
        glm::mat4 view = glm::lookAt(glm::vec3(0,2,-4), glm::vec3(0,0,0), glm::vec3(0.0f,1.0f,0.0f));
        m_V = view;
        MVP = m_P * m_V * M;
        MV = m_V * M;
        N = glm::inverse(glm::mat3(MV));
        ...
        if(i==2)
        {
            ...
            activeTexture=1;
            glUniform1i(glGetUniformLocation(pid, "activeTexture"),activeTexture);
...
////    glUniform1i(glGetUniformLocation(pid, "colourTexFromFBo1"), 3);
////    glUniform1i(glGetUniformLocation(pid, "depthTexFromFBo1"), 4);
...
//    glUniformMatrix4fv(glGetUniformLocation(pid, "MV"), 1, false, glm::value_ptr(MVP));
